...
%--------------------------------------------------------------------------
Zhs=mt*(((4*eta)-(2*(eta^2)))/((1-eta)^3));
%--------------------------------------------------------------------------
tum1=0;
for i=1:nc
  tum1=tum1+(x(i)*(1-m(i))*(((5*eta)-(2*(eta^2)))/((1-eta)*(2-eta))));  
end
Zchain=tum1;
%--------------------------------------------------------------------------
sum1=0;
...
%--------------------------------------------------------------------------
ghs=(1-(eta/2))/((1-eta)^3);
delta=(sigma(2)^3)*ghs*kappa*(exp(epsilon/T)-1);
XA=(-1+sqrt(1+(8*Na*ro*x(2)*delta)))/(4*Na*ro*x(2)*delta);
ass1=1+(((5*eta)-(2*(eta^2)))/((1-eta)*(2-eta)));
