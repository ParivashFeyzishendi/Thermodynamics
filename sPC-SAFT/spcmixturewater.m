...
%-------
%CH4+H2O
x=[0.000261795945771748     0.999738204054228];
y=[0.996674513832901       0.00332548616709902];
Mw=[16 18];
%---------
R=8.314;
Na=6.02*10^23;
...
%kij=0;
qe=[0 kij;kij 0];
sigma=[3.686*10^-10 2.1328*10^-10];
epsk=[148.84 163.94];
m=[1.012 2.521];
kappa=0.2712;
epsilon=1575.73;
...
%--------------------------------------------------------------------------
deltal=(sigma(2)^3)*gl*kappa*(exp(epsilon/T)-1);
XAl=(-1+sqrt(1+(8*Na*rol*x(2)*deltal)))/(4*Na*rol*x(2)*deltal)
for i=1:nc
  assl(i)=coef*m(i)*(d(i)^3)*((5-(2*etal))/((1-etal)*(2-etal)))
end
...
%--------------------------------------------------------------------------
deltav=(sigma(2)^3)*gv*kappa*(exp(epsilon/T)-1);
XAv=(-1+sqrt(1+(8*Na*rov*y(2)*deltav)))/(4*Na*rov*y(2)*deltav);
for i=1:nc
  assv(i)=coef*m(i)*(d(i)^3)*((5-(2*etav))/((1-etav)*(2-etav)));
end
