...
kij=0.05;
qe=[0 kij;kij 0];
sigma=[3.7574*10^-10 3.2300*10^-10];
epsk=[216.53 188.90];
m=[2.2616 1.5255];
kappa=0.035176;
epsilon=2899.5;
...
muassl(1)=-1*rol*x(2)*(1-XAl)*assl(1);
muassl(2)=(2*log(XAl))-(1*rol*x(2)*(1-XAl)*assl(2));
%--------------------------------------------------------------------------
%Vapor
tumv0=0;
tumv1=0;
...
muassv(1)=-1*rov*y(2)*(1-XAv)*assv(1);
muassv(2)=(2*log(XAv))-(1*rov*y(2)*(1-XAv)*assv(2));
%--------------------------------------------------------------------------
for i=1:nc
