...
%kij=0.05;
qe=[0 kij;kij 0];
sigma=[2.17*10^-10 5.263269547*10^-10];
epsk=[207.6515802 374.9931652];
m=[2.237853198 8.173351805];
kappa=0.002821724;
epsilon=3994.382699;
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
deltal=(d(2)^3)*gl(2)*kappa*(exp(epsilon/T)-1);
XAl=(-1+sqrt(1+(4*Na*rol*x(2)*deltal)))/(2*Na*rol*x(2)*deltal);
for i=1:nc 
   j=2; 
   sl(i)=((d(i)^3)/(1-zetal3)^2)+((1.5*d(j)*(d(i)^2))/(1-zetal3)^2)+((3*d(j)*(d(i)^3)*zetal2)/(1-zetal3)^3)+(((d(j)^2)*(d(i)^2)*zetal2)/(1-zetal3)^3)+((1.5*(d(j)^2)*(d(i)^3)*(zetal2^2))/(1-zetal3)^4);      
   assl(i)=(((pi/6)*m(i)*Na)*(sl(i)/gl(2)));
end
muassl(1)=-2*rol*x(2)*(1-XAl)*assl(1);
muassl(2)=(4*log(XAl))-(2*rol*x(2)*(1-XAl)*assl(2));
%--------------------------------------------------------------------------
%Vapor
tumv0=0;
tumv1=0;
...
%--------------------------------------------------------------------------
deltav=(d(2)^3)*gv(2)*kappa*(exp(epsilon/T)-1);
XAv=(-1+sqrt(1+(4*Na*rov*y(2)*deltav)))/(2*Na*rov*y(2)*deltav);
for i=1:nc 
   j=2; 
   sv(i)=((d(i)^3)/(1-zetav3)^2)+((1.5*d(j)*(d(i)^2))/(1-zetav3)^2)+((3*d(j)*(d(i)^3)*zetav2)/(1-zetav3)^3)+(((d(j)^2)*(d(i)^2)*zetav2)/(1-zetav3)^3)+((1.5*(d(j)^2)*(d(i)^3)*(zetav2^2))/(1-zetav3)^4);      
   assv(i)=(((pi/6)*m(i)*Na)*(sv(i)/gv(2)));
end
muassv(1)=-2*rov*y(2)*(1-XAv)*assv(1);
muassv(2)=(4*log(XAv))-(2*rov*y(2)*(1-XAv)*assv(2));
%--------------------------------------------------------------------------
for i=1:nc
